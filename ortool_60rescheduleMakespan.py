...
#output_json('./timeline/{}/{}/60/{}/{}_{}.json'.format(f_data[2], f_data[3], f_data[-2], f_data[-1][:-4], _round), m_cp)
    ...
    return machine_occupied, get_makespan(machine_num, machine_occupied, job_due_date)
...
def get_makespan(machine_num, machine_occupied, job_due_date):
...
def solve(env, time_limit=60, num_thread=1, enable_sol_printer=False, objective='tardiness'):
    ...
    for job_id in all_jobs:
        if job_ends[job_id] is None:
            continue
        adj_job_ends.append(job_ends[job_id])
    model.AddMaxEquality(makespan, adj_job_ends)
    ...
    for job_id in all_jobs:
        for op_id in range(len(fjsp_data[job_id])):
            ...
            makespan_result = max(makespan_result, start_value + duration)
            result.append(op_info)
...
#                result_dir = os.path.join('./result', dir_name, str(time_limit), 'thread5')
                ...
                for file in os.listdir(os.path.join(data_dir, dir_name, dir_size)):
                    ...
                    if ext != '.fjs':
                        continue
                    result, makespan = solver(os.path.join(data_dir, dir_name, dir_size, file), timeline_dir, time_limit=time_limit, num_thread=num_thread, DDT=args.DDT, objective=args.objective)
                    with open(os.path.join(result_dir, 'or-tools_result{}.txt'.format(args.logname)), 'a') as outfile:
                        outfile.write('{} : {}\n'.format(os.path.join(dir_name, dir_size, file), makespan))
