...
def solver(path, time_limit=60, num_thread=1, enable_sol_printer=False):
    print(path)
...
#        output_json('./timeline/Base_mk04/seed_1855_newjob_Tarr=30_breakdown/300/{}/{}_{}.json'.format(path.split('/')[-2], path.split('/')[-1][:-4], _round), m_cp)
        f_data = path.split('/')
#output_json('./timeline/{}/{}/60/{}/{}_{}.json'.format(f_data[2], f_data[3], f_data[-2], f_data[-1][:-4], _round), m_cp)
        output_json('./timeline/{}/{}/{}/{}/60/{}/postpone_{}_{}.json'.format(f_data[2], f_data[3], f_data[4], f_data[5], f_data[-2], f_data[-1][:-4], _round), m_cp)
        ...
        while len(event_holder) and event_holder[0][1] == current_time:
            ...
            if dynamic_event[0] == 'new job':
                ...
                if len(machine_occupied[machine_id]) and (machine_occupied[machine_id][-1].Proto().start.offset <= breakdown_start and breakdown_start < machine_occupied[machine_id][-1].Proto().end.offset):
                    postpone_op = machine_occupied[machine_id].pop()

                if postpone_op:
                    start, end = postpone_op.Proto().start.offset, postpone_op.Proto().end.offset
                    job_id, op_id = map(int, postpone_op.Name().split('_')[:2])
                    op_name = str(job_id) + '_' + str(op_id)
                    op_before = model.NewIntervalVar(start, breakdown_start - start, breakdown_start, op_name)
                    op_after = model.NewIntervalVar(breakdown_start + breakdown_duration, end - breakdown_start, end + breakdown_duration, op_name)
                    
                    machine_occupied[machine_id].append(op_before)
                    machine_occupied[machine_id].append(breakdown)
                    machine_occupied[machine_id].append(op_after)
...
def solve(env, time_limit=60, num_thread=1, enable_sol_printer=False):
...
if __name__ == '__main__':
    ...
    gen_type = ['MK_1215or/'+args.targetdir]
    # event_type = ['breakdown', 'newjob', 'newjob_breakdown']
    # event_type = ['newjob_breakdown_seed_6404']
#    event_type = [
    ...
    for g_type in gen_type:
        for e_type in event_type:
            ...
            for dir_size in os.listdir(os.path.join(data_dir, dir_name)):
                result_dir = os.path.join('./result', dir_name, str(time_limit), 'thread1')
                ...
                for file in os.listdir(os.path.join(data_dir, dir_name, dir_size)):
                    ...
                    if ext != '.fjs':
                        continue
                    result, tard = solver(os.path.join(data_dir, dir_name, dir_size, file), time_limit=time_limit, num_thread=num_thread)
                    with open(os.path.join(result_dir, 'or-tools_result_pos.txt'), 'a') as outfile:
                        outfile.write('{} : {}\n'.format(os.path.join(dir_name, dir_size, file), tard))
                    output_json('{}_pos.json'.format(os.path.join(timeline_dir, file)[:-4]), result)
