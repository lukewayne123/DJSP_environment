            ...
            for instance in sorted(os.listdir(size_set)):
                ...
                for cnt in range(args.test_sample_times):
                    ...
                    while True:
                        ...
                        if done:
                            if best_makespan > env.get_makespan():
                                best_makespan = env.get_makespan()
                            ...
                            print("instance : {}, tard : {}, makespan: {}".format(file, env.get_tardiness(), env.get_makespan()))
                            break

                with open('./result/{}/test_result.txt'.format(args.date),"a") as outfile:
#outfile.write(f'instance : {file:50} tard : {best_tard:10} \n')
                    outfile.write(f'instance : {file:50} util_sys : {best_util_sys:4} util_idv : {best_util_idv:4} tard: {best_tard:10} tardy_num: {best_tardy_num:10} tardy_rate: {best_tardy_rate:10} makespan: {best_makespan:10}\n')
                with open('./result/{}/test_result_detail.csv'.format(args.date),"a") as csvfile:
                    writer = csv.writer(csvfile)
#writer.writerow([file[-6:], best_util_sys, best_util_idv, best_tard])
                    writer.writerow([file[-6:], best_tard, best_tardy_num, best_tardy_rate, best_util_sys, best_util_idv, best_makespan, last_arr])

                avg_makespan += best_makespan
                avg_tard += best_tard
            ...
            print(f'instance : {file[:-6]:44}, AVG tard : {avg_tard//10:10}, AVG makespan : {avg_makespan//10:10}')
            with open('./result/{}/test_result.txt'.format(args.date),"a") as outfile:
                outfile.write(f'instance : {file[:-6]:44} AVG tard : {avg_tard//10:10} AVG makespan : {avg_makespan//10:10}\n')
            with open('./result/{}/test_result_avg.csv'.format(args.date),"a") as csvfile:
                writer = csv.writer(csvfile)
#writer.writerow([file[:-6], avg_sys_util / 10.0, avg_sys_util_idv / 10.0, avg_tard/10.0])
                writer.writerow([file[:-6], avg_tard/10.0, avg_tardy_num/10.0, avg_tardy_rate/10.0, avg_sys_util / 10.0, avg_sys_util_idv / 10.0, avg_makespan / 10.0])
...
#policy.load_state_dict(torch.load('./weight/RS+op_FJSP/best', map_location=args.device), False)
#    policy.load_state_dict(torch.load('./weight/0507_ScNwAwBMWKRddt3makespan/198000', map_location=args.device), False)
    policy.load_state_dict(torch.load('./weight/0611_ScNwAwBMWKRddt3makespan/147000', map_location=args.device), False)
